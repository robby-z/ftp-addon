...
Select **Open Web UI** on the App page to manage recordings inside Home Assistant. The responsive default table combines all camera directories, orders recordings newest first, and supports camera, text, date, quick-age, and watched-state filters, sortable columns, and pagination. The header reports utilization of the actual validated recording filesystem. Each row can be viewed, downloaded, marked watched/unwatched, or deleted; deletion is also available in the player. The original per-user folder view remains available in the **Ordner** tab.

Use the row checkboxes to select individual recordings or the header checkbox for the current page. The selection bar can expand that selection to every result matching the active filters, while still allowing individual exclusions. Bulk deletion always displays the affected count and expected reclaimed space and requires an explicit irreversible-action confirmation. Bulk download is intentionally not provided.

Home Assistant Ingress authenticates the browser and proxies its internal port; port 8099 is not published to the LAN. Symbolic links and paths outside the configured user roots are refused. Deletion additionally requires a per-start CSRF token and is limited to stable video files already present in the index, so an active upload cannot be removed. Before deleting all filter results, the server recomputes and verifies both confirmed file count and byte size. The rebuildable SQLite index is stored under App `/data`; it contains metadata and the shared watched state only, while recordings remain on the configured media disk. Direct MP4 playback depends on browser codec support. H.264 is broadly compatible, while H.265/HEVC may require downloading the original recording and opening it in a compatible player.
